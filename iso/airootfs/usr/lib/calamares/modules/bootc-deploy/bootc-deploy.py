...
def run():
    ...
    # Replace it with a clean config that searches for the boot partition.
    ...
    # Load bootuuid.cfg from ESP to get the boot partition UUID
    bootuuid_cfg = os.path.join(esp_arch, "bootuuid.cfg")
    if os.path.exists(bootuuid_cfg):
        with open(bootuuid_cfg) as f:
            bootuuid_content = f.read().strip()
        # bootuuid.cfg format: set BOOT_UUID="xxxx-xxxx"
        boot_uuid = bootuuid_content.split('"')[1] if '"' in bootuuid_content else ""
    else:
        boot_uuid = ""

    if boot_uuid:
        search_cmd = f'search --fs-uuid "{boot_uuid}" --set boot --no-floppy'
    else:
        search_cmd = 'search --label boot --set boot --no-floppy'

    grub_cfg_content = f"""# Generated by bootc-deploy (replaces bootupd's RAID-detecting stub)
{search_cmd}
set root=$boot
if [ -d ($boot)/grub ]; then
