...
def run():
    ...
    # Read BLS entries and generate a direct-boot config (blscfg creates duplicates)
            ...
            with open(conf) as f:
                for line in f:
                    ...
                    if idx > 0:
                        ...
                        entry[key] = val
            if entry.get("linux"):
                linux_cmd = f"linux {entry['linux']} {entry.get('options', '')}"
                if entry.get("initrd"):
                    initrd_cmd = f"initrd {entry['initrd']}"
                break
